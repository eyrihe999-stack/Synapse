...
var (
	// ErrEmbeddingInvalid 请求参数非法(如空 input、维度不符等),不可重试。
	ErrEmbeddingInvalid = errors.New("embedding: invalid request")
	// ErrEmbeddingAuth API key 错误 / 权限不足,重试无用,需 operator 介入。
	ErrEmbeddingAuth = errors.New("embedding: authentication failed")
	// ErrEmbeddingRateLimited 触发 provider 速率限制,可退避重试(Azure 会带 Retry-After)。
	ErrEmbeddingRateLimited = errors.New("embedding: rate limited")
	// ErrEmbeddingServer provider 侧 5xx 错误,暂时性故障,可 backoff 重试。
	ErrEmbeddingServer = errors.New("embedding: server error")
	// ErrEmbeddingNetwork 请求发送失败(DNS / TCP / 超时),可 backoff 重试。
	ErrEmbeddingNetwork = errors.New("embedding: network error")
	// ErrEmbeddingDimMismatch provider 返回的向量维度与配置不符,说明部署错了模型,fatal。
