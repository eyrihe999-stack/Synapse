...
var (
	// ErrInvalidDatacenterID 构造 Snowflake 时 datacenter_id 越界(合法范围 0-31)。
	ErrInvalidDatacenterID = errors.New("datacenter ID must be between 0 and 31")
	// ErrInvalidWorkerID 构造 Snowflake 时 worker_id 越界(合法范围 0-31)。
	ErrInvalidWorkerID = errors.New("worker ID must be between 0 and 31")
	// ErrClockBackwards 系统时钟倒流(NTP 矫正等),拒绝生成 ID 防止重复。
	ErrClockBackwards = errors.New("clock moved backwards, refusing to generate ID")
	// ErrSequenceOverflow 同一毫秒内序列号耗尽(4096 个/ms 上限)。
	ErrSequenceOverflow = errors.New("sequence overflow in the same millisecond")
)

// Snowflake 生成全局唯一 ID 的发号器。基于 Twitter Snowflake 算法,ID 由 timestamp + datacenter_id + worker_id + sequence 构成。
...
// SnowflakeConfig Snowflake 构造参数。两 ID 传 -1 或 0 时自动从环境/K8s/IP/MAC 推导。
