...
type Orchestrator struct {
	...
	topOrchestratorPID uint64 // 启动时一次性查出,不变
...
//
// 失败场景:
//   - 查不到 top-orchestrator agent 行(PR #4' 的 seed 应该已写入,找不到说明迁移没跑)
//
...
func NewOrchestrator(
	...
	topAgent, err := agentRepo.FindByAgentID(ctx, agents.TopOrchestratorAgentID)
	if err != nil {
		return nil, fmt.Errorf("locate top-orchestrator agent: %w", err)
	}
	if topAgent.PrincipalID == 0 {
		return nil, errors.New("top-orchestrator agent principal_id is 0 — PR #4' migration not applied?")
	}
	...
	return &Orchestrator{
		...
		usageRepo:          usageRepo,
		logger:             log,
		topOrchestratorPID: topAgent.PrincipalID,
		consumerName:       consumerName,
	}, nil
}

// TopOrchestratorPrincipalID 暴露给测试 / 外部校验;业务代码不应该读它。
func (o *Orchestrator) TopOrchestratorPrincipalID() uint64 { return o.topOrchestratorPID }
...
func (o *Orchestrator) Run(ctx context.Context) error {
	if err := o.consumer.EnsureGroup(ctx, o.cfg.ChannelStream, ConsumerGroupName); err != nil {
		return fmt.Errorf("ensure consumer group: %w", err)
	}

	if deleted, skipped, err := o.consumer.SweepIdleConsumers(
		ctx, o.cfg.ChannelStream, ConsumerGroupName, idleConsumerSweepThreshold,
	); err != nil {
		o.logger.WarnCtx(ctx, "agentsys: sweep idle consumers failed", map[string]any{
			"stream": o.cfg.ChannelStream, "group": ConsumerGroupName, "err": err.Error(),
		})
	...
	if n <= 1 {
		o.logger.InfoCtx(ctx, "agentsys: top-orchestrator runtime starting", map[string]any{
			...
			"top_orchestrator_pid": o.topOrchestratorPID,
			"concurrency":          1,
		})
		...
		return o.consumer.Consume(ctx, o.cfg.ChannelStream, ConsumerGroupName, o.consumerName, o.handleEvent)
	}
	...
	for i := 0; i < n; i++ {
		...
		if err := runner.Go(ctx, consumerName, func(rctx context.Context) {
			...
			if err := o.consumer.Consume(rctx, o.cfg.ChannelStream, ConsumerGroupName, consumerName, o.handleEvent); err != nil {
				select {
				case errCh <- err:
				default:
				}
...
func (o *Orchestrator) cleanupConsumer(name string) {
	...
	defer cancel()
	if err := o.consumer.DeleteConsumer(cleanupCtx, o.cfg.ChannelStream, ConsumerGroupName, name); err != nil {
		o.logger.WarnCtx(cleanupCtx, "agentsys: cleanup delete consumer failed", map[string]any{
			"stream": o.cfg.ChannelStream, "group": ConsumerGroupName, "consumer": name, "err": err.Error(),
		})
		return
	}
	o.logger.InfoCtx(cleanupCtx, "agentsys: consumer deleted on shutdown", map[string]any{
		"stream": o.cfg.ChannelStream, "group": ConsumerGroupName, "consumer": name,
	})
}
...
func (o *Orchestrator) handleEvent(ctx context.Context, msg eventbus.Message) error {
	...
	if authorPID != 0 && authorPID == o.topOrchestratorPID {
		return nil
	}

	// 3. @ 检测 —— 未被 @ 顶级 agent 就直接 ACK(不响应、不写 audit、不耗 token)
	mentions := parseMentionCSV(msg.Fields["mentioned_principal_ids"])
	if !containsPID(mentions, o.topOrchestratorPID) {
		return nil
	}
	...
	s := scoped.NewForTrigger(orgID, channelID, o.topOrchestratorPID,
